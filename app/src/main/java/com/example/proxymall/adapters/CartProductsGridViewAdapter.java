...
public class CartProductsGridViewAdapter extends BaseAdapter {
	...
	public CartProductsGridViewAdapter(Context context,
...
//				intent.putExtra("from", "commerce");
				intent.putExtra("productId", resultp.get(AppConfig.TAG_PRODUCT_ID));
				intent.putExtra("name", resultp.get(AppConfig.TAG_NAME));
				intent.putExtra("price", resultp.get(AppConfig.TAG_PRICE));
				intent.putExtra("category", resultp.get(AppConfig.TAG_CATEGORY));
				...
				intent.putExtra("mallAddress", resultp.get(AppConfig.TAG_MALL_ADDRESS));
